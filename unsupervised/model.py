...
def inference(image_tensor, is_training, need_featuremap=False):
    config = {}
    ...
    layer10 = do_conv(layer9, 'layer10-conv7', kernel_size=5, depth=256, stride_size=1, padding='SAME', config=config,
                      activation_method=activation_method)
    layer11 = do_conv(layer10, 'layer11-conv8', kernel_size=5, depth=256, stride_size=1, padding='SAME', config=config,
                      activation_method=activation_method)

    layer12 = do_pooling(layer11, 'max', 'layer12-maxpooling4', kernel_size=2, stride_size=2, padding='VALID')
    print layer12
    conv_output = flatten22Dim(layer12)

    fc1 = do_fc(conv_output, layer_name='layer13-fc1', depth=128, activation_method=activation_method)

    fc2 = do_fc(fc1, layer_name='layer14-fc2', depth=1000, activation_method=activation_method)

    fc3 = do_fc(fc2, layer_name='layer15-fc3', depth=784, activation_method=activation_method)

    fc4 = do_fc(fc3, layer_name='layer16-fc4', depth=2, activation_method=activation_method)
    # if train:
...
if __name__ == '__main__':
    ...
    center_feature, adversarial_feature = inference(image_tensor, is_training=True, need_featuremap=True)
    print 'adversarial_feature: ', adversarial_feature
